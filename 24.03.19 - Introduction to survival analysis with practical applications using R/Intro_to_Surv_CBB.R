...
# CBB, 2024/03/19
# Integrated and adapted by Davide Valentini (CBB @KI /CAST @KS)
# from STHDA educational guides (http://www.sthda.com/)
...
library("survival")
library("survminer", dep=T)

# IMPORT OR LOAD DATA
# One can use the drop-down menu or the R command
# the same dataset can be loaded from a folder, local or from the web. 
...
fitaa <- survfit(Surv(time, status) ~ as.factor(sex), data = lung)
...
#  Plot the scaled Schoenfeld residuals against the transformed time 
  ...
  #the model. The smoothed plot is thus an estimate 
