...
greentea.data <-
  caffeine.data %>%
  dplyr::filter(drink=="greentea") # I am using dplyr::filter() here to indicate I want the filter() command from the dplyr package (and not from another package)
View(greentea.data)
...
female.coffee.data <-
  ...
  dplyr::filter(
    drink=="coffee",
    gender=="female") # I am using dplyr::filter() here to indicate I want the filter() command from the dplyr package (and not from another package)
View(female.coffee.data)
...
# Export as image ####
# pdf
ggsave("P:/laura.vossen/Marie_Lof/CBB_support_sessions/Session5/Histogram_age.pdf", width=4, height=6)
